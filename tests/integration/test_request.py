...
def test_successful_non_streaming_request(test_config, function_manager, provider_manager, preset_manager):
...
def test_execute_llm_streaming_failure_call_llm(test_config, function_manager, provider_manager, preset_manager):
    request = make_api_request(test_config, function_manager, provider_manager, preset_manager, request_overrides={'stream': True})
    request.set_request_llm()
    new_messages, messages = request.prepare_ask_request()
    request.llm = Mock()
...
def test_post_response_full_function_run(test_config, function_manager, provider_manager, preset_manager):
...
def test_request_with_function_call_in_streaming_mode(test_config, function_manager, provider_manager, preset_manager):
    preset = preset_manager.presets['test']
    function_responses = [
        [
            AIMessageChunk(content='', additional_kwargs={'function_call': {'name': 'test_function', 'arguments': ''}}),
            AIMessageChunk(content='', additional_kwargs={'function_call': {'arguments': '{\n  "word": "foo",\n  "repeats": 2\n}'}}),
        ],
        [
...
def test_post_response_return_on_function_call(test_config, function_manager, provider_manager, preset_manager):
    preset = preset_manager.presets['test']
    function_responses = [
        AIMessage(content='', additional_kwargs={'function_call': {'name': 'test_function', 'arguments': '{\n  "word": "foo"\n}'}}),
    ]
...
def test_post_response_return_on_function_response(test_config, function_manager, provider_manager, preset_manager):
    preset = preset_manager.presets['test']
    function_responses = [
        AIMessage(content='', additional_kwargs={'function_call': {'name': 'test_function', 'arguments': '{\n  "word": "foo",\n  "repeats": 2\n}'}}),
        "Foo repeated twice is: foo foo",
...
def test_post_response_multiple_function_calls(test_config, function_manager, provider_manager, preset_manager):
    preset = preset_manager.presets['test']
    function_responses = [
        AIMessage(content='', additional_kwargs={'function_call': {'name': 'test_function', 'arguments': '{\n  "word": "foo",\n  "repeats": 2\n}'}}),
        AIMessage(content='', additional_kwargs={'function_call': {'name': 'test_function', 'arguments': '{\n  "word": "foo",\n  "repeats": 2\n}'}}),
...
def test_post_response_forced_function_call(test_config, function_manager, provider_manager, preset_manager):
    preset = preset_manager.presets['test']
    function_responses = [
        AIMessage(content='', additional_kwargs={'function_call': {'name': 'test_function', 'arguments': '{\n  "word": "foo",\n  "repeats": 2\n}'}}),
...
def test_request_exceeds_max_submission_tokens(test_config, function_manager, provider_manager, preset_manager):
    # Set max_submission_tokens to a low value
    request = make_api_request(test_config, function_manager, provider_manager, preset_manager, max_submission_tokens=1)
    request.set_request_llm()
