...
logger.basicConfig(format='[%(asctime)s] %(message)s', datefmt='%I:%M:%S %p')
...
class AsyncChatGPT:
    ...
    async def init(self, headless: bool = True, browser="firefox", timeout=60, proxy: Optional[ProxySettings] = None):
        ...
        logger.basicConfig(format='[%(asctime)s] [JS] %(message)s', datefmt='%I:%M:%S %p')
    ...
    async def refresh_session(self,timeout=20,remaining_retry=5):
        ...
        # Try again
        await self.refresh_session(remaining_retry=remaining_retry-1)
        return
    ...
    async def ask_stream(self, prompt: str, remaining_retry=5):
    ...
    async def ask_stream_clicking(self, prompt: str, remaining_retry=5):
    ...
    async def ask(self, message: str, remaining_retry=5) -> str:
        """
...
class ChatGPT(AsyncChatGPT):
    ...
    def refresh_session(self,timeout=20,remaining_retry=5):
        return asyncio.run(super().refresh_session(timeout=timeout,remaining_retry=remaining_retry))
        ...
        def iter_over_async(ait):
            ...
            while True:
                ...
                if done:
                    break
                yield obj
        yield from iter_over_async(super().ask_stream(prompt=prompt,remaining_retry=remaining_retry))
    def ask(self,message:str,remaining_retry=5) -> str:
        return asyncio.run(super().ask(message=message,remaining_retry=remaining_retry))
